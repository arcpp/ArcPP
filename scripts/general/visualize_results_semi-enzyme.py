...
def main(results_pkl=None, fdr_pkl=None, protein_pkl=None, peptide_pkl=None, plot_types=[]):
    ...
    if 'plot_cleavage_sites' in plot_types:
        print('plotting graph and table for predicted and identified N-termini')
        inference_dict = protein_inference(results_dict)
        plot_cleavage_sites(
            results_dict,
            output_filename='Cleavage_sites.svg',
            prot_info_dict=prot_info_dict,
            inference_dict=inference_dict,
            spec_filter=5,
            dataset_filter=1,
...
def plot_cleavage_sites(
    ...
    '''
    Signal peptide cleavage sites predicted by e.g. SignalP 5.0 are compared
    to semi-enzymatic peptides identified within ArcPP.
    '''
    ...
    with open(db_fasta, 'r') as db_input:
        for fasta_id, db_sequence in ursgal.ucore.parse_fasta(db_input):
            ...
            if len(db_sequence) <= 60:
                ...
                prot_term_dict[fasta_id] = {
                    'N-term': db_sequence[:60],
                    'C-term': db_sequence[-60:],
                    'full': db_sequence,
                    'N-term_full_peps': {},
                    'N-term_semi_peps': {},
                }
            prot_term_dict[fasta_id]['predicted_cs'] = int(prot_info_dict[org_fasta_id]['signalp_new_n'])
            # prot_term_dict[fasta_id]['predicted_cs'] = int(prot_info_dict[org_fasta_id]['phobius_new_n'])
            # if prot_info_dict[org_fasta_id]['pred_tat_SP'] is not False:
            #     prot_term_dict[fasta_id]['predicted_cs'] = int(prot_info_dict[org_fasta_id]['pred_tat_new_n'])
            #     # print('works')
            # elif prot_info_dict[org_fasta_id]['pred_sig_SP'] is not False:
            #     prot_term_dict[fasta_id]['predicted_cs'] = int(prot_info_dict[org_fasta_id]['pred_sig_new_n'])
                # print('works too')
            # else:
            #     print(prot_info_dict[org_fasta_id]['pred_sig_SP'])
            #     prot_term_dict[fasta_id]['predicted_cs'] = 1
            # prot_term_dict[fasta_id]['decision_tree'] = prot_info_dict[org_fasta_id]['decision_tree']
            if prot_info_dict[org_fasta_id]['signalp'] == 'TAT(Tat/SPI)':
                if prot_info_dict[org_fasta_id]['tatlipo'] is True:
                    prot_term_dict[fasta_id]['signalp'] = 'TAT(Tat/lipobox)'
                else:
                    prot_term_dict[fasta_id]['signalp'] = prot_info_dict[org_fasta_id]['signalp']    
            else:        
                prot_term_dict[fasta_id]['signalp'] = prot_info_dict[org_fasta_id]['signalp']
    ...
    for prot in sorted(results_dict['all']['protein_dict'].keys()):
        ...
        for seq in results_dict['all']['protein_dict'][prot].keys():
            ...
            for n, p in enumerate(prot.split('<|>')):
                min_start = int(start[n].split(';')[0])
                semi = results_dict['all']['protein_dict'][prot][seq]['semi']
                psms = results_dict['all']['protein_dict'][prot][seq]['spec_title']
                datasets = results_dict['all']['protein_dict'][prot][seq]['datasets']
                if semi == 'full':
                    if min_start not in prot_term_dict[p]['N-term_full_peps'].keys():
                        prot_term_dict[p]['N-term_full_peps'][min_start] = defaultdict(list)
                    prot_term_dict[p]['N-term_full_peps'][min_start][seq] = [psms, datasets]
                if len(psms) <= spec_filter:
                    continue
                if len(datasets) <= dataset_filter:
                    continue
                elif semi == 'semi':
                    if min_start == 1:
                        continue
                    if prot_term_dict[p]['full'][min_start-2] in ['K', 'R']:
                        continue
                    if len(prot_term_dict[p]['full']) == min_start+len(seq)-1:
                        if min_start not in prot_term_dict[p]['N-term_semi_peps'].keys():
                            prot_term_dict[p]['N-term_semi_peps'][min_start] = defaultdict(list)
                        prot_term_dict[p]['N-term_semi_peps'][min_start][seq] = [psms, datasets]
                    elif prot_term_dict[p]['full'][min_start+len(seq)-1] in ['P']:
                        continue
                    else:
                        if min_start not in prot_term_dict[p]['N-term_semi_peps'].keys():
                            prot_term_dict[p]['N-term_semi_peps'][min_start] = defaultdict(list)
    ...
    uncleaved_set = set()
    multiple_cs_set = set()
    potential_cs_set = set()
    unique_cs_set = set()
    predicted_cs = set()
    total = set()
    for prot in prot_term_dict.keys():
        total.add(prot)
        full = sorted(prot_term_dict[prot]['N-term_full_peps'].keys())
        semi = sorted(prot_term_dict[prot]['N-term_semi_peps'].keys())
        semi_N_term = set()
        for s in semi:
            if s < 60:
                semi_N_term.add(s)
        if prot_term_dict[prot]['predicted_cs'] > 3:
            predicted_cs.add(prot)
        if len(full) == 0 and len(semi) == 0:
            undetected_set.add(prot)
            continue
        if len(semi_N_term) == 0:
            if len(full) == 0:
                undetected_set.add(prot)
                continue
            if min(full) <= 3:
                uncleaved_set.add(prot)
            else:
                continue
        elif len(semi_N_term) > 1:
            if len(full) == 0:
                continue
            elif min(semi) < min(full) and len(full) > len(semi):
                potential_cs_set.add(prot)
            multiple_cs_set.add(prot)
            continue
        else:
            if len(full) == 0:
                continue
                # unique_cs_set.add(prot)
            elif min(semi) < min(full) and len(full) > len(semi):
                unique_cs_set.add(prot)
            else:
                multiple_cs_set.add(prot)
    print('Total number of proteins:', len(total))
    ...
    fieldnames = [
        ...
        'Datasets',
        'Predicted CS',
        'Prediction Localization/Processing',
    ]
    ...
    out_csv_name = 'Cleavage_sites_predicted.csv'
    with open(out_csv_name, 'w') as csv_out:
        ...
        for prot in prot_term_dict:
            if prot_term_dict[prot]['predicted_cs'] == 1 and prot_term_dict[prot]['signalp'] is False:
                continue
            if prot_term_dict[prot]['predicted_cs'] == 1 or prot_term_dict[prot]['signalp'] is False:
                print(prot)
                exit(1)
            for start in prot_term_dict[prot]['N-term_semi_peps'].keys():
                if start > 60:
                    continue
                for seq in prot_term_dict[prot]['N-term_semi_peps'][start].keys():
                    out_dict = {}
                    out_dict['Sequence'] = seq
                    out_dict['Enzyme Specificity'] = 'semi'
                    out_dict['Protein ID'] = prot
                    out_dict['Sequence Start'] = start
                    out_dict['Number of PSMs'] = len(prot_term_dict[prot]['N-term_semi_peps'][start][seq][0])
                    datasets = ';'.join(sorted(prot_term_dict[prot]['N-term_semi_peps'][start][seq][1]))
                    out_dict['Datasets'] = datasets
                    out_dict['Predicted CS'] = prot_term_dict[prot]['predicted_cs']
                    # out_dict['Prediction Localization/Processing'] = prot_term_dict[prot]['decision_tree']
                    out_dict['Prediction Localization/Processing'] = prot_term_dict[prot]['signalp']
                    csv_writer.writerow(out_dict)
            for start in prot_term_dict[prot]['N-term_full_peps'].keys():
                if start > 60:
                    continue
                for seq in prot_term_dict[prot]['N-term_full_peps'][start].keys():
                    out_dict = {}
                    out_dict['Sequence'] = seq
                    out_dict['Enzyme Specificity'] = 'full'
                    out_dict['Protein ID'] = prot
                    out_dict['Sequence Start'] = start
                    out_dict['Number of PSMs'] = len(prot_term_dict[prot]['N-term_full_peps'][start][seq][0])
                    datasets = ';'.join(sorted(prot_term_dict[prot]['N-term_full_peps'][start][seq][1]))
                    out_dict['Datasets'] = datasets
                    out_dict['Predicted CS'] = prot_term_dict[prot]['predicted_cs']
                    # out_dict['Prediction Localization/Processing'] = prot_term_dict[prot]['decision_tree']
                    out_dict['Prediction Localization/Processing'] = prot_term_dict[prot]['signalp']
    ...
    color_dict = {
        ...
        'Sec; SPI': '#d9ef8b',
        'Sec; lipobox': '#a6d96a',
        'Identified CS': 'rgb(125,125,125)',
        'Predicted CS': 'rgb(125,125,125)',
        'SP(Sec/SPI)':'#4363d8',
        'TAT(Tat/SPI)':'#f58231',
        'TAT(Tat/lipobox)':'#800000',
        'LIPO(Sec/SPII)':'#911eb4',
    }

    total_too_long_short = set()
    total_correct = set()
    total_different_cs = set()
    total_wrong = set()
    total_count = set()
    datadict_list = []
    for category in [
        ...
        no_evidence = set()
        uncleaved_ident = set()
        uncleaved_before_cs = set()
        different_cs = set()
        correct_cs = set()
        full_pep_semi_pep = set()
        full_N_semi_around = set()
        full_before_semi_around = set()
        pred_cs_on_try = set()
        new_N_too_short = set()
        new_N_too_long = set()
        for prot in predicted_cs:
            # if category is 'Other':
            #     if prot_term_dict[prot]['decision_tree'] not in ['Pil; SPIII', 'Cyt', '>=2 TM', 'TM C-term', 'TM N-term', '1 TM']:
            #         continue
            # else:
                # if prot_term_dict[prot]['decision_tree'] != category:
                #     continue
            if prot_term_dict[prot]['signalp'] != category:
                continue
            pred_cs = prot_term_dict[prot]['predicted_cs']
            full = sorted(prot_term_dict[prot]['N-term_full_peps'].keys())
            semi = sorted(prot_term_dict[prot]['N-term_semi_peps'].keys())
            if len(full)==0 and len(semi)==0:
                no_evidence.add(prot)
                total_count.add(prot)
                continue
            if pred_cs in semi:
                pred_cs_psms = 0
                for pep in prot_term_dict[prot]['N-term_semi_peps'][pred_cs].keys():
                    pred_cs_psms += len(prot_term_dict[prot]['N-term_semi_peps'][pred_cs][pep][0])
                correct_cs.add(prot)
                for cs in full:
                    if cs > pred_cs:
                        continue
                    if len(semi) >= len(full):
                        continue
                    print('Full Pep and CS semi pep:', prot)
                    full_pep_semi_pep.add(prot)
                    # for pep in prot_term_dict[prot]['N-term_full_peps'][cs].keys():
                    #     if len(prot_term_dict[prot]['N-term_full_peps'][cs][pep][0]) >= pred_cs_psms:
                    #         print('Full Pep PSMS > CS PSMS:', prot)
                total_count.add(prot)
                continue
            new_cs = False
            for s in semi:
                if s >= pred_cs-3 and s <= pred_cs+3:
                    if len(semi) >= len(full):
                        continue
                    new_cs = True
                    for cs in full:
                        if cs > min(semi):
                            continue
                        print('Full before and semi around CS (1):', prot)
                        full_before_semi_around.add(prot)
                        if len(semi) > 1:
                            new_cs = False
                    if 'LAGC' in prot_term_dict[prot]['full'] and\
                        min(semi) > prot_term_dict[prot]['full'].index('LAGC')+3:
                        new_cs = False
            if new_cs:
                different_cs.add(prot)
                total_count.add(prot)
                continue
            if len(semi) == 1 and min(semi) < 60:
                new_cs = False
                if list(semi)[0] >= pred_cs-3 and list(semi)[0] <= pred_cs+3:
                    new_cs = True
                for cs in full:
                    if cs > min(semi):
                        continue
                    print('Full Pep and different CS semi pep:', prot)
                    full_pep_semi_pep.add(prot)
                    new_cs = False
                    if cs < pred_cs-3:
                        uncleaved_before_cs.add(prot)
                    else:
                        no_evidence.add(prot)
                if 'LAGC' in prot_term_dict[prot]['full'] and\
                        min(semi) > prot_term_dict[prot]['full'].index('LAGC')+3:
            ...
            for n in range(1,4):
                if n in full:
                    uncleaved_ident.add(prot)
                    uncleaved = True
            if uncleaved:
                for s in semi:
                    if s >= pred_cs-3 and s <= pred_cs+3:
                        print('Full N-term and semi around CS:', prot)
                        full_N_semi_around.add(prot)
                if prot_term_dict[prot]['full'][pred_cs-2] in ['R', 'K']:
                    print('Pred CS on trypsin CS:', prot)
                    pred_cs_on_try.add(prot)
                if 'K' in prot_term_dict[prot]['full'][pred_cs-1:pred_cs+5] or \
                    'R' in prot_term_dict[prot]['full'][pred_cs-1:pred_cs+5]:
                    # print('Full N but semi too short:', prot, prot_term_dict[prot]['decision_tree'])
                    new_N_too_short.add(prot)
                if 'K' not in prot_term_dict[prot]['full'][pred_cs-1:pred_cs+50] and \
                    'R' not in prot_term_dict[prot]['full'][pred_cs-1:pred_cs+50]:
                    # print('Full N but semi too long:', prot, prot_term_dict[prot]['decision_tree'])
                    new_N_too_long.add(prot)
                total_count.add(prot)
                continue
            uncleaved_before = False
            # if len(semi) == 0:
            for cs in full:
                if cs >= pred_cs-3:
                    continue
                uncleaved_before_cs.add(prot)
                uncleaved_before = True
                for s in semi:
                    if s >= pred_cs-3 and s <= pred_cs+3:
                        print('Full before and semi around CS (2):', prot)
                        full_before_semi_around.add(prot)
                if 'K' in prot_term_dict[prot]['full'][pred_cs-1:pred_cs+5] or \
                    'R' in prot_term_dict[prot]['full'][pred_cs-1:pred_cs+5]:
                    # print('Full before but semi too short:', prot, prot_term_dict[prot]['decision_tree'])
                    new_N_too_short.add(prot)
                if 'K' not in prot_term_dict[prot]['full'][pred_cs-1:pred_cs+50] and \
                    'R' not in prot_term_dict[prot]['full'][pred_cs-1:pred_cs+50]:
                    # print('Full before but semi too long:', prot, prot_term_dict[prot]['decision_tree'])
                    new_N_too_long.add(prot)
            if uncleaved_before is False:
                no_evidence.add(prot)
                total_count.add(prot)
                continue
            else:
        ...
        datadict = {
            'x': [
                # 'No evidence',
                # 'Evidence for no SP',
                # 'Incorrect CS (full before CS)',
                # 'Incorrect CS (different CS)',
                # 'Evidence for correct CS',
                'Tryptic peptide<br>start < CS - 3 aa',
                'Semi-tryptic peptide<br>start at CS +/- 3 aa',
                'Semi-tryptic peptide<br>start at CS',
            ],
            'y': [
                # len(no_evidence),
                # len(uncleaved_ident - new_N_too_long - new_N_too_short),
                # len(uncleaved_before_cs - new_N_too_long - new_N_too_short),
                len((uncleaved_ident | uncleaved_before_cs) - new_N_too_long - new_N_too_short),
                len(different_cs),
                len(correct_cs),
            ],
            'text': [],
        ...
        print(correct_cs)
        # print('''
        #     New N too long/short: {0} {1}
        #     '''.format(
        #     len(new_N_too_long),
        #     len(new_N_too_short)
        # ))
        total_too_long_short |= (new_N_too_long | new_N_too_short)
        total_wrong |= ((uncleaved_ident | uncleaved_before_cs) - new_N_too_long - new_N_too_short)
        total_different_cs |= different_cs
        total_correct |= correct_cs
        # print(uncleaved_before_cs - new_N_too_long - new_N_too_short)
    print('Total predicted:',len(predicted_cs))    
    print('Total sorted into cats:',len(total_count))
    print('Total too long/short:',len(total_too_long_short))
    print('Total wrong:',len(total_wrong))
    print('Total different:',len(total_different_cs))
    print('Total correct:',len(total_correct))

    pprint.pprint(total_wrong)

    plot_barplot(
        datadict_list=datadict_list,
        output_filename=output_filename.replace('.svg', '_predicted_barplot.pdf'),
        layout=layout,
